...
def plot_map_contourf_aod(lon, lat, mnasa, mec, nasa, ec, nodabckg, dabckg, daanal, \
    ...
    ptitle='550 nm Aerosol Optical Depth (AOD) \n on %s/%s/%s' % (cm, cd, cy)
    fsize1=14
    fsize2=14
    fsize3=14
    ...
    pname='NASA-ECMWF-AOD_full_0m_f000.png'
    plt.savefig(pname)
    plt.close(fig)
    return
...
if __name__ == '__main__':
    ...
    ncind=open_ncfile(merra2)
    lat=ncind.variables['lat'][:]
    lon=ncind.variables['lon'][:]
    merra2_aod=ncind.variables['series_cnt_OBAR'][:]
    fcst_aod=ncind.variables['series_cnt_FBAR'][:]
    fcst_merra2_bias=ncind.variables['series_cnt_ME'][:]
    fcst_merra2_rmse=ncind.variables['series_cnt_MSE'][:]
    fcst_merra2_rmse=np.sqrt(fcst_merra2_rmse)
    ncind.close()

    ncind=open_ncfile(cams)
    #lat=ncind.variables['lat'][:]
    #lon=ncind.variables['lon'][:]
    cams_aod=ncind.variables['series_cnt_OBAR'][:]
    #fcst_aod=ncind.variables['series_cnt_FBAR'][:]
    fcst_cams_bias=ncind.variables['series_cnt_ME'][:]
    fcst_cams_rmse=ncind.variables['series_cnt_MSE'][:]
    fcst_cams_rmse=np.sqrt(fcst_cams_rmse)
    ncind.close()
    ...
    nasamod='NASA/MERRA2'
    ecmod='ECMWF/CAMS-ERA4'
    fcstmod=f'fcst at {fhr} lead time'
    ...
    pp = plot_map_contourf_8(lon, lat, merra2_aod, cams_aod, fcst_aod, 
                           fcst_merra2_bias, fcst_merra2_rmse, \
                           fcst_cams_bias, fcst_cams_rmse, \
	                   nasamod, ecmod, fcstmod, cmapaod, cmapbias, cmaprmse)
