...
def plot_line(data, plegs, pcols, plines, pmarks, xarr, xlab, ylab, ypert, pstr):
    ...
    nx, ny = data.shape
    for iy in range(ny):
        plt.plot(xarr, data[:,iy], linewidth=2, marker=pmarks[irun], linestyle=plines[irun], color=pcols[irun],label=plegs[irun])
        irun+=1
    ...
    for obstype in obstypes:
        ...
        for exprun in expruns:
            ...
            for fhr in range(fhmin, fhmax, fhout):
                ...
                infile = f'{exprun}_{obstype}_FBAR_OBAR_ME_RMSE_MAE_PRCORR_{fhrstr}.txt'
        ...
        xarr=str(range(fhmin, fhmax, fhout))
        xlab="Forecast (days)"
        ypert= 0
        for ix in range(nx):
            data = arr3d[ix,:,:]
            if ix == 0: pstr = 'FBAR'; ylab='Model Mean'
            if ix == 1: pstr = 'OBAR'; ylab='Reanalysis Mean'
            if ix == 2: pstr = 'ME';   ylab='Mean Bias'
            if ix == 3: pstr = 'RMSE'; ylab='Mean RMSE'
            if ix == 4: pstr = 'MAE';  ylab='Mean Abs. Error'
            if ix == 5: pstr = 'PRCORR'; ylab='PR Correlation'
            ypert=0
