...
!mzp march, 2020
  ...
  CHARACTER(len = max_varname_length) :: varname,varname_out
...
!< unit: pascal  
  ...
  DO iv=1,nvars_in
     ...
     CASE('bc1','bc2','oc1','oc2','sulf','so4',&
        ...
        WHERE (vardataijll > aero_max) vardataijll=aero_max
        units="kg kg-1"
        validrange=[0.,aero_max]
     CASE("T")
        units="K"
        validrange=[100.,400.]
     CASE("sphum")
...
!     ENDDO
     ...
     CALL translate_fv3gocart2reanalysis(varname,varname_out)

     CALL netcdf_write_generic_ll(.FALSE.,llfile,&
          &varname=varname_out,vardata=vardataijll,units=units,&
          &validrange=validrange)
