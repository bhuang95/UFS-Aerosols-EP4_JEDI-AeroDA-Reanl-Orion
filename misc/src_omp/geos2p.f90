...
!mzp june, 2020
  ...
  CHARACTER(len=max_string_length) :: input_geos_dir,fname_geos,fname_akbk,&
  ...
  NAMELIST /record_input/ date,input_geos_dir,fname_geos,fname_akbk
  ...
  INQUIRE(file='geos2p.nl', exist=isfile)
  IF ( isfile ) THEN
     OPEN(unit=98, file='geos2p.nl', &
     ...
     CLOSE(98)
  ELSE
     PRINT *,'Missing namelist geos2p.nl'
     STOP(1)
  ENDIF
  ...
  akbkfile=fname_akbk
  fname_geos=TRIM(input_geos_dir)//"/"//TRIM(fname_geos)
...
!  PRINT *,MINVAL(bk),MAXVAL(bk)
  ...
  CALL read_m2_var(fname_geos,varname,vardataij)
...
!  ENDDO
  ...
  DO iv=1,nvars_out
     ...
     IF ( (INDEX(upper2lower(varnames_out(iv)),"du") > 0) ) THEN
        ...
        CALL aggregate_m2gocart2cams(fname_geos,"dust",varnames_in,&
     ...
     ELSEIF ( (INDEX(upper2lower(varnames_out(iv)),"seas") > 0) ) THEN
        ...
        CALL aggregate_m2gocart2cams(fname_geos,"seas",varnames_in,&
             &varnames_out(iv),vardataij)
        CALL p_interp_nx_ny(pmid,plevels,vardataij,vardataijp)
        CALL netcdf_write_generic_pll(.FALSE.,pllfile,&
             &varname=varnames_out(iv),vardata=vardataijp)
