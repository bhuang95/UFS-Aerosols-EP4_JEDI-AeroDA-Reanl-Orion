...
PROGRAM convert_gbbepx_noco
  ...
  REAL, POINTER, DIMENSION(:,:) :: ebc=>NULL(), eoc=>NULL(), &
       &epm25=>NULL(), eso2=>NULL(), plume=>NULL()
  ...
  OPEN(unit=unit_namelist, file = "convert_gbbepx_noco.nl", &
       &status="old",action = "READ",iostat=open_status)
  IF (open_status /= 0) THEN
     WRITE(stderr,*) "error: missing NAMELIST file (convert_gbbepx_noco.nl)"
     STOP
  END IF
...
END PROGRAM convert_gbbepx_noco
