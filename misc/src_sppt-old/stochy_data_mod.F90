...
!  if stochini is true, then read in pattern from a file
   ...
   if (nocnskeb > 0) then
      ...
      call patterngenerator_init(ocnskeb_lscale(1:nocnskeb),ocnskebint,ocnskeb_tau(1:nocnskeb),ocnskeb(1:nocnskeb),iseed_ocnskeb,rpattern_ocnskeb, &
           lonf,latg,jcap,gis_stochy_ocn%ls_node,nocnskeb,1,1,new_lscale)
      do n=1,nocnskeb
         if (stochini) then
...
! normalize SKEB by equation 8 of Berner et al. 2009
         do locl=1,ls_max_node
            l = gis_stochy_ocn%ls_node(locl,1)
            jbasev = gis_stochy_ocn%ls_node(locl,2)
            indev = indlsev(l,l)
            jbasod = gis_stochy_ocn%ls_node(locl,3)
            indod = indlsod(l+1,l)
            print*,'4*pi*radius**2)/rpattern_ocnskeb(n)%phi',pi,radius,rpattern_ocnskeb(n)%phi
            do n2=l,jcap,2
               rnn1 = n2*(n2+1.)*(2*n2+1)
               gis_stochy_ocn%kenorm_e(indev) = rnn1/(4*pi*radius**2)/rpattern_ocnskeb(n)%phi
               indev = indev + 1
            enddo
            do n2=l+1,jcap,2
               rnn1 = n2*(n2+1.)*(2*n2+1)
               gis_stochy_ocn%kenorm_o(indod) = rnn1/(4*pi*radius**2)/rpattern_ocnskeb(n)%phi
               indod = indod + 1
            enddo
         enddo
      enddo
      if (is_rootpe()) print*,'normalizing ocean skeb ',maxval(gis_stochy_ocn%kenorm_e(:)),minval(gis_stochy_ocn%kenorm_e(:))
   endif
   deallocate(noise_e,noise_o)
