...
# these are the variables that can be used to match up channels
cha_vars = [
    'MetaData/sensorCentralFrequency',
    'MetaData/sensorCentralWavelength',
...
def combine_obsspace(FileList, OutFile, GeoDir):
    ...
    for f in FileList:
        ...
        for vname in obsspace.variables:
            ...
            elif vname in cha_vars:
                if vname not in ChaVarNames:
                    ChaVarNames.append(vname)
                    VarAttrFiles[vname] = f
            else:
    ...
    # output variables
    #LocKeyList = []
    LocKeyList = [("latitude", "float"), ("longitude", "float"), ("dateTime", "string")]
    DimDict = {}
    ...
    for fullvname in ChaVarNames:
        gname, vname = fullvname.split('/')
        _var = obsspace.Variable(fullvname)
        tmpdata = np.array(_var.read_data())
        OutData[(vname, gname)] = tmpdata
        VarDims[fullvname] = ['Channel']

    DimDict['Channel'] = np.array([4], dtype=np.intc)
    nchas = 1
    ...
    for fullvname in LocVarNames + AllVarNames:
        ...
        VarDims[fullvname] = ['Channel']
        #if fullvname in LocVarNames:
        #    VarDims[fullvname] = ['Location']
        #else:
        #    VarDims[fullvname] = ['Location', 'Channel']
        del _var
        del obsspace
...
#            del obsspace
    ...
    """
    # extract channel specific variables and generate a numpy array
    MetaVarData = []
    bad_idxs = []
    for vname in ChaVarNames:
        tmpvardata = []
        #if LocInAll[vname]:
        if 1==1:
            #for f in FileList[0]:
    ...
    MetaVarUnique, idx, inv, cnt = np.unique(MetaVarData, return_index=True,
                                             return_inverse=True, return_counts=True, axis=1)
    DimDict['Channel'] = len(idx)
    nchas = len(idx)
    for idx2, fullvname in enumerate(ChaVarNames):
        gname, vname = fullvname.split('/')
        OutData[(vname, gname)] = MetaVarUnique[idx2, ...].astype(VarTypes[fullvname])
    """
    ...
    for fullvname in AllVarNames:
        ...
        for f in FileList:
            ...
            if fullvname in obsspace.variables:
                _var = obsspace.Variable(fullvname)
                tmpdata = np.array(_var.read_data())
                #tmpdata = np.reshape(np.array(_var.read_data()),(nlocs, nchas))
                print('HBO1-tmpdata-shape')
                print(tmpdata.shape)
            else:
    ...
    for idx2, fullvname in enumerate(AllVarNames):
        ...
        print('HBO2-DataVarUnique-fullvname')
        print(fullvname)
        print(mask)
        print(DataVarData[idx2, mask].shape)
        print(DataVarUnique[idx2, inv[mask]].shape)
