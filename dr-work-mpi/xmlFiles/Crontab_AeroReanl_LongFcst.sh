...
hh=$(date +%H)
exehrs="01 03 05 07 09 11 13 15 17 19 21 23"
for hr in ${exehrs}; do
if [ "${hh}" = "${hr}" ]; then
/home/bohuang/Workflow/UFS-Aerosols_NRTcyc/UFS-Aerosols-EP4_JEDI-AeroDA-Reanl-Orion/misc/Globus/job_globus_her2niag_longfcst.sh
fi
done
...
for hr in ${exehrs}; do
if [ "${hh}" = "${hr}" ]; then
/home/bohuang/Workflow/UFS-Aerosols_NRTcyc/UFS-Aerosols-EP4_JEDI-AeroDA-Reanl-Orion/misc/Globus/job_resubmit_globus_loop_herculesRun_longfcst.sh
fi
done
